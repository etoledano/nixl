...
nixl_status_t
nixlGdsIOBatch::addToBatch(
        CUfileHandle_t fh, void* buffer, size_t size, size_t file_offset, size_t ptr_offset, CUfileOpcode_t type)
{
...
nixlGdsEngine::nixlGdsEngine(const nixlBackendInitParams* init_params) : nixlBackendEngine(init_params)
{
...
nixl_status_t
nixlGdsEngine::registerMem(const nixlBlobDesc& mem, const nixl_mem_t& nixl_mem, nixlBackendMD*& out)
{
...
nixl_status_t
nixlGdsEngine::checkXfer(nixlBackendReqH* handle)
{
    ...
    for (auto it = gds_handle->batch_io_list.begin(); it != gds_handle->batch_io_list.end();) {
...
nixl_status_t
nixlGdsEngine::releaseReqH(nixlBackendReqH* handle)
{
    nixlGdsBackendReqH* gds_handle = (nixlGdsBackendReqH*)handle;
