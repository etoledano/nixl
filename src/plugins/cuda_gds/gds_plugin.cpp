...
create_gds_engine(const nixlBackendInitParams* init_params)
{
    return new nixlGdsEngine(init_params);
}

static void
destroy_gds_engine(nixlBackendEngine* engine)
{
    delete engine;
}
...
static nixlBackendPlugin plugin = {NIXL_PLUGIN_API_VERSION, create_gds_engine,   destroy_gds_engine, get_plugin_name,
