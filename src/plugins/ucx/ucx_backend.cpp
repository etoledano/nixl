...
int
nixlUcxEngine::vramUpdateCtx(void* address, uint32_t devId, bool& restart_reqd)
{
    int ret;
    bool was_updated;
...
void
nixlUcxEngine::_requestInit(void* request)
{
...
void
nixlUcxEngine::_requestFini(void* request)
{
    /* Finalize request */
...
void
nixlUcxEngine::progressThreadStart()
{
...
void
nixlUcxEngine::progressThreadStop()
{
    if (!pthrOn) {
        // not enabled
        return;
...
void
nixlUcxEngine::progressThreadRestart()
{
    progressThreadStop();
    progressThreadStart();
}
...
nixlUcxEngine::nixlUcxEngine(const nixlBackendInitParams* init_params) : nixlBackendEngine(init_params)
{
    ...
    uc = new nixlUcxContext(devs, sizeof(nixlUcxBckndReq), _requestInit, _requestFini, NIXL_UCX_MT_WORKER);
...
nixl_status_t
nixlUcxEngine::endConn(const std::string& remote_agent)
{
...
ucs_status_t
nixlUcxEngine::connectionCheckAmCb(
...
ucs_status_t
nixlUcxEngine::connectionTermAmCb(
        void* arg, const void* header, size_t header_length, void* data, size_t length,
        const ucp_am_recv_param_t* param)
{
    struct nixl_ucx_am_hdr* hdr = (struct nixl_ucx_am_hdr*)header;
...
nixlUcxEngine::connect(const std::string& remote_agent)
{
    ...
    ret = uw->sendAm(
            conn.ep, CONN_CHECK, &hdr, sizeof(struct nixl_ucx_am_hdr), (void*)localAgent.data(), localAgent.size(),
            flags, req);
...
nixlUcxEngine::disconnect(const std::string& remote_agent)
{
    ...
    if (remote_agent != localAgent) {
        ...
        ret = uw->sendAm(
                conn.ep, DISCONNECT, &hdr, sizeof(struct nixl_ucx_am_hdr), (void*)localAgent.data(), localAgent.size(),
                flags, req);
...
nixl_status_t
nixlUcxEngine::loadRemoteConnInfo(const std::string& remote_agent, const std::string& remote_conn_info)
{
 ...
 *****************************************/
nixl_status_t
nixlUcxEngine::registerMem(const nixlBlobDesc& mem, const nixl_mem_t& nixl_mem, nixlBackendMD*& out)
{
    int ret;
...
// To be cleaned up
nixl_status_t
nixlUcxEngine::internalMDHelper(const nixl_blob_t& blob, const std::string& agent, nixlBackendMD*& output)
{
    nixlUcxConnection conn;
...
// To be cleaned up
nixl_status_t
nixlUcxEngine::loadRemoteMD(
        const nixlBlobDesc& input, const nixl_mem_t& nixl_mem, const std::string& remote_agent, nixlBackendMD*& output)
{
...
nixl_status_t
nixlUcxEngine::retHelper(nixl_status_t ret, nixlUcxBckndReq* head, nixlUcxReq& req)
{
...
nixl_status_t
nixlUcxEngine::checkXfer(nixlBackendReqH* handle)
{
...
nixl_status_t
nixlUcxEngine::releaseReqH(nixlBackendReqH* handle)
{
    nixlUcxBckndReq* head = (nixlUcxBckndReq*)handle;
    nixlUcxBckndReq* req = head;
...
nixl_status_t
nixlUcxEngine::notifSendPriv(const std::string& remote_agent, const std::string& msg, nixlUcxReq& req)
{
    nixlSerDes ser_des;
    std::string* ser_msg;
    nixlUcxConnection conn;
    ...
    ret = uw->sendAm(
            conn.ep, NOTIF_STR, &hdr, sizeof(struct nixl_ucx_am_hdr), (void*)ser_msg->data(), ser_msg->size(), flags,
            req);
...
void
nixlUcxEngine::notifCombineHelper(notif_list_t& src, notif_list_t& tgt)
{
    if (!src.size()) {
        // Nothing to do. Exit
        return;
...
void
nixlUcxEngine::notifProgressCombineHelper(notif_list_t& src, notif_list_t& tgt)
{
    notifMtx.lock();
...
nixl_status_t
nixlUcxEngine::genNotif(const std::string& remote_agent, const std::string& msg)
{
    nixl_status_t ret;
    nixlUcxReq req;
