...
nixlBackendEngine*
nixlPluginHandle::createEngine(const nixlBackendInitParams* init_params)
{
...
void
nixlPluginHandle::destroyEngine(nixlBackendEngine* engine)
{
...
std::shared_ptr<nixlPluginHandle>
nixlPluginManager::loadPluginFromPath(const std::string& plugin_path)
{
    ...
    if (!init) {
        std::cerr << "Failed to find nixl_plugin_init in " << plugin_path << ": " << dlerror() << std::endl;
        dlclose(handle);
        return nullptr;
    }
    ...
    if (plugin->api_version != NIXL_PLUGIN_API_VERSION) {
        std::cerr << "Plugin API version mismatch for " << plugin_path << ": expected " << NIXL_PLUGIN_API_VERSION
...
std::shared_ptr<nixlPluginHandle>
nixlPluginManager::loadPlugin(const std::string& plugin_name)
{
...
nixlPluginManager::discoverPluginsFromDir(const std::string& dirpath)
{
    ...
    while ((entry = readdir(dp))) {
        ...
        if (filename.substr(0, 10) == "libplugin_" && filename.substr(filename.size() - 3) == ".so") {
            // Extract plugin name
...
void
nixlPluginManager::unloadPlugin(const nixl_backend_t& plugin_name)
{
...
std::shared_ptr<nixlPluginHandle>
nixlPluginManager::getPlugin(const nixl_backend_t& plugin_name)
{
    auto it = loaded_plugins_.find(plugin_name);
    if (it != loaded_plugins_.end()) {
        return it->second;
...
nixl_b_params_t
nixlPluginHandle::getBackendOptions()
{
...
nixl_mem_list_t
nixlPluginHandle::getBackendMems()
{
...
nixlPluginManager::getLoadedPluginNames()
{
...
std::vector<nixlStaticPluginInfo>&
nixlPluginManager::getStaticPlugins()
{
