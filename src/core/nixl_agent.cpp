...
nixlAgentData::nixlAgentData(const std::string& name, const nixlAgentConfig& cfg) : name(name), config(cfg)
{
...
nixl_status_t
nixlAgent::getPluginParams(const nixl_backend_t& type, nixl_mem_list_t& mems, nixl_b_params_t& params) const
{
...
nixl_status_t
nixlAgent::getBackendParams(const nixlBackendH* backend, nixl_mem_list_t& mems, nixl_b_params_t& params) const
{
    if (!backend)
        return NIXL_ERR_INVALID_PARAM;
...
nixl_status_t
nixlAgent::createBackend(const nixl_backend_t& type, const nixl_b_params_t& params, nixlBackendH*& bknd_hndl)
{
...
nixl_status_t
nixlAgent::prepXferDlist(
        const std::string& agent_name, const nixl_xfer_dlist_t& descs, nixlDlistH*& dlist_hndl,
        const nixl_opt_args_t* extra_params) const
{
    ...
    for (auto& backend : *backend_set) {
        ...
        if (init_side)
            ...
            ret = data->remoteSections[agent_name]->populate(descs, backend, *(handle->descs[backend]));
        if (ret == NIXL_SUCCESS) {
            count++;
        } else {
...
nixlAgent::makeXferReq(
        const nixl_xfer_op_t& operation, const nixlDlistH* local_side, const std::vector<int>& local_indices,
        const nixlDlistH* remote_side, const std::vector<int>& remote_indices, nixlXferReqH*& req_hndl,
        const nixl_opt_args_t* extra_params) const
{
    ...
    if ((desc_count == 0) || (remote_indices.size() == 0) || (desc_count != (int)remote_indices.size()))
        return NIXL_ERR_INVALID_PARAM;
        ...
        while (i < (desc_count)) {
            ...
            if (i != (desc_count - 1)) {
                ...
                while (((local_desc1.addr + local_desc1.len) == local_desc2->addr) &&
                       ...
                       (local_desc1.devId == local_desc2->devId) && (remote_desc1.devId == remote_desc2->devId)) {
    ...
    ret = handle->engine->prepXfer(
            handle->backendOp, *handle->initiatorDescs, *handle->targetDescs, handle->remoteAgent,
            handle->backendHandle, &opt_args);
    if (ret != NIXL_SUCCESS) {
        delete handle;
        return ret;
    }
...
nixl_status_t
nixlAgent::createXferReq(
        const nixl_xfer_op_t& operation, const nixl_xfer_dlist_t& local_descs, const nixl_xfer_dlist_t& remote_descs,
        const std::string& remote_agent, nixlXferReqH*& req_hndl, const nixl_opt_args_t* extra_params) const
{
    ...
    if (!extra_params || extra_params->backends.size() == 0) {
        ...
        backend_set_t* remote_set = data->remoteSections[remote_agent]->queryBackends(remote_descs.getType());
    ...
    for (auto& backend : *backend_set) {
        ...
        ret2 = data->remoteSections[remote_agent]->populate(remote_descs, backend, *handle->targetDescs);
...
nixlAgent::postXferReq(nixlXferReqH* req_hndl, const nixl_opt_args_t* extra_params) const
{
    ...
    ret = req_hndl->engine->postXfer(
            req_hndl->backendOp, *req_hndl->initiatorDescs, *req_hndl->targetDescs, req_hndl->remoteAgent,
            req_hndl->backendHandle, &opt_args);
    req_hndl->status = ret;
    return ret;
}
...
nixl_status_t
nixlAgent::genNotif(const std::string& remote_agent, const nixl_blob_t& msg, const nixl_opt_args_t* extra_params)
{
    nixlBackendEngine* backend = nullptr;
    backend_list_t* backend_list;
...
nixlAgent::invalidateRemoteMD(const std::string& remote_agent)
{
    ...
    if (data->remoteBackends.count(remote_agent) != 0) {
        for (auto& elm : data->remoteBackends[remote_agent]) data->backendEngines[elm]->disconnect(remote_agent);
