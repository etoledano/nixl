...
nixl_status_t
nixlMemSection::populate(const nixl_xfer_dlist_t& query, nixlBackendEngine* backend, nixl_meta_dlist_t& resp) const
{
...
nixl_reg_dlist_t
nixlLocalSection::getStringDesc(const nixlBackendEngine* backend, const nixl_meta_dlist_t& d_list) const
{
    nixl_status_t ret;
...
nixlLocalSection::addDescList(
        const nixl_reg_dlist_t& mem_elms, nixlBackendEngine* backend, nixl_meta_dlist_t& remote_self)
{
    if (!backend)
        return NIXL_ERR_INVALID_PARAM;
    ...
    for (int i = 0; i < mem_elms.descCount(); ++i) {
        ...
        *lp = mem_elms[i];  // Copy the basic desc part
        if (((nixl_mem == BLK_SEG) || (nixl_mem == OBJ_SEG) || (nixl_mem == FILE_SEG)) && (lp->len == 0))
...
nixl_status_t
nixlLocalSection::remDescList(const nixl_meta_dlist_t& mem_elms, nixlBackendEngine* backend)
{
    if (!backend)
        return NIXL_ERR_INVALID_PARAM;
    nixl_mem_t nixl_mem = mem_elms.getType();
...
nixl_status_t
nixlRemoteSection::loadLocalData(const nixl_meta_dlist_t& mem_elms, nixlBackendEngine* backend)
{
