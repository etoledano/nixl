...
nixlBasicDesc::nixlBasicDesc(const uintptr_t& addr, const size_t& len, const uint32_t& dev_id)
{
    this->addr = addr;
    this->len = len;
    this->devId = dev_id;
...
bool
nixlBasicDesc::operator<(const nixlBasicDesc& desc) const
{
    if (devId != desc.devId)
...
bool
nixlBasicDesc::covers(const nixlBasicDesc& query) const
{
    if (devId == query.devId) {
        if ((addr <= query.addr) && (addr + len >= query.addr + query.len))
            return true;
    }
    return false;
}
...
bool
nixlBasicDesc::overlaps(const nixlBasicDesc& query) const
{
    if (devId != query.devId)
        return false;
...
nixl_blob_t
nixlBasicDesc::serialize() const
{
    return std::string(reinterpret_cast<const char*>(this), sizeof(nixlBasicDesc));
}

void
nixlBasicDesc::print(const std::string& suffix) const
{
...
nixlBlobDesc::nixlBlobDesc(
        const uintptr_t& addr, const size_t& len, const uint32_t& dev_id, const nixl_blob_t& meta_info)
    : nixlBasicDesc(addr, len, dev_id)
{
    this->metaInfo = meta_info;
}

nixlBlobDesc::nixlBlobDesc(const nixlBasicDesc& desc, const nixl_blob_t& meta_info) : nixlBasicDesc(desc)
{
    this->metaInfo = meta_info;
}
...
nixl_blob_t
nixlBlobDesc::serialize() const
{
    return nixlBasicDesc::serialize() + metaInfo;
}

void
nixlBlobDesc::copyMeta(const nixlBlobDesc& info)
{
    this->metaInfo = info.metaInfo;
}

void
nixlBlobDesc::print(const std::string& suffix) const
{
...
template <class T>
inline const T&
nixlDescList<T>::operator[](unsigned int index) const
{
    if (index >= descs.size())
...
template <class T>
inline T&
nixlDescList<T>::operator[](unsigned int index)
{
    if (index >= descs.size())
        throw std::out_of_range("Index is out of range");
    sorted = false;
...
template <class T>
void
nixlDescList<T>::addDesc(const T& desc)
{
    if (!sorted) {
...
template <class T>
bool
nixlDescList<T>::overlaps(const T& desc, int& index) const
{
    if (!sorted) {
...
template <class T>
bool
nixlDescList<T>::hasOverlaps() const
{
...
template <class T>
void
nixlDescList<T>::remDesc(const int& index)
{
...
template <class T>
nixl_status_t
nixlDescList<T>::populate(const nixlDescList<nixlBasicDesc>& query, nixlDescList<T>& resp) const
{
...
template <class T>
int
nixlDescList<T>::getIndex(const nixlBasicDesc& query) const
{
    if (!sorted) {
...
template <class T>
nixl_status_t
nixlDescList<T>::serialize(nixlSerDes* serializer) const
{
    nixl_status_t ret;
    ...
    if (std::is_same<nixlBasicDesc, T>::value) {
        ...
        ret = serializer->addStr(
                "", std::string(reinterpret_cast<const char*>(descs.data()), n_desc * sizeof(nixlBasicDesc)));
        if (ret)
            return ret;
...
template <class T>
bool
operator==(const nixlDescList<T>& lhs, const nixlDescList<T>& rhs)
{
    if ((lhs.getType() != rhs.getType()) || (lhs.descCount() != rhs.descCount()) || (lhs.isSorted() != rhs.isSorted()))
        return false;
