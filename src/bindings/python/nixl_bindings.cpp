...
PYBIND11_MODULE(_bindings, m)
{
    ...
    py::class_<nixl_xfer_dlist_t>(m, "nixlXferDList")
            ...
            .def(py::init([](nixl_mem_t mem, std::vector<py::tuple> descs, bool sorted) {
                     ...
                     for (long unsigned int i = 0; i < descs.size(); i++)
                         new_list[i] = nixlBasicDesc(
                                 descs[i][0].cast<uintptr_t>(), descs[i][1].cast<size_t>(),
                                 descs[i][2].cast<uint32_t>());
                     if (sorted)
            ...
            .def(py::self == py::self)
            .def("__getitem__",
                 [](nixl_xfer_dlist_t& list, unsigned int i) -> std::tuple<uintptr_t, size_t, uint32_t> {
            ...
            .def("__setitem__",
                 [](nixl_xfer_dlist_t& list, unsigned int i, const py::tuple& desc) {
                     list[i] =
                             nixlBasicDesc(desc[0].cast<uintptr_t>(), desc[1].cast<size_t>(), desc[2].cast<uint32_t>());
                 })
            .def("addDesc",
                 [](nixl_xfer_dlist_t& list, const py::tuple& desc) {
                     list.addDesc(nixlBasicDesc(
                             desc[0].cast<uintptr_t>(), desc[1].cast<size_t>(), desc[2].cast<uint32_t>()));
                 })
            .def("append",
                 [](nixl_xfer_dlist_t& list, const py::tuple& desc) {
                     list.addDesc(nixlBasicDesc(
    ...
    py::class_<nixl_reg_dlist_t>(m, "nixlRegDList")
            ...
            .def(py::init([](nixl_mem_t mem, std::vector<py::tuple> descs, bool sorted) {
                     ...
                     for (long unsigned int i = 0; i < descs.size(); i++)
                         new_list[i] = nixlBlobDesc(
                                 descs[i][0].cast<uintptr_t>(), descs[i][1].cast<size_t>(),
                                 descs[i][2].cast<uint32_t>(), descs[i][3].cast<std::string>());
                     if (sorted)
                         new_list.verifySorted();
                     return new_list;
                 }),
            ...
            .def(py::self == py::self)
            .def("__getitem__",
                 [](nixl_reg_dlist_t& list, unsigned int i) -> std::tuple<uintptr_t, size_t, uint32_t, std::string> {
    ...
    py::class_<nixlAgent>(m, "nixlAgent")
            ...
            .def("getPluginParams",
                 [](nixlAgent& agent,
                    const nixl_backend_t type) -> std::pair<nixl_b_params_t, std::vector<std::string>> {
                     ...
                     for (const auto& elm : mems) mems_vec.push_back(nixlEnumStrings::memTypeStr(elm));
                     return std::make_pair(params, mems_vec);
                 })
            .def("getBackendParams",
                 [](nixlAgent& agent, uintptr_t backend) -> std::pair<nixl_b_params_t, std::vector<std::string>> {
                     nixl_b_params_t params;
                     nixl_mem_list_t mems;
                     std::vector<std::string> mems_vec;
            ...
            .def("createBackend",
                 [](nixlAgent& agent, const nixl_backend_t& type, const nixl_b_params_t& initParams) -> uintptr_t {
            ...
            .def(
                    "registerMem",
                    [](nixlAgent& agent, nixl_reg_dlist_t descs, std::vector<uintptr_t> backends) -> nixl_status_t {
                        nixl_opt_args_t extra_params;
                        nixl_status_t ret;
                        for (uintptr_t backend : backends) extra_params.backends.push_back((nixlBackendH*)backend);
            ...
            .def(
                    "createXferReq",
                    [](nixlAgent& agent, const nixl_xfer_op_t& operation, const nixl_xfer_dlist_t& local_descs,
                       const nixl_xfer_dlist_t& remote_descs, const std::string& remote_agent,
                       const std::string& notif_msg, std::vector<uintptr_t> backends) -> uintptr_t {
                    ...
                    py::arg("operation"), py::arg("local_descs"), py::arg("remote_descs"), py::arg("remote_agent"),
                    py::arg("notif_msg") = std::string(""), py::arg("backend") = std::vector<uintptr_t>({}))
                    ...
                    [](nixlAgent& agent, const nixl_xfer_op_t& operation, uintptr_t local_side,
                       ...
                       bool skip_desc_merge) -> uintptr_t {
                        ...
                        throw_nixl_exception(agent.makeXferReq(
                                operation, (nixlDlistH*)local_side, local_indices, (nixlDlistH*)remote_side,
                                remote_indices, handle, &extra_params));

                        return (uintptr_t)handle;
                    },
                    py::arg("operation"), py::arg("local_side"), py::arg("local_indices"), py::arg("remote_side"),
            ...
            .def("getNotifs",
                 [](nixlAgent& agent, nixl_py_notifs_t& notif_map) -> nixl_py_notifs_t {
                     ...
                     for (const auto& pair : new_notifs) {
                         for (const auto& str : pair.second) notif_map[pair.first].push_back(py::bytes(str));
                     }
                     return notif_map;
                 })
            .def(
