...
checkCudaError(cudaError_t result, const char* message)
{
    if (result != cudaSuccess) {
        std::cerr << message << " (Error code: " << result << " - " << cudaGetErrorString(result) << ")" << std::endl;
        exit(EXIT_FAILURE);
    }
}
#endif
...
void
releaseEngine(nixlBackendEngine* ucx)
{
    delete ucx;
}
...
allocateBuffer(nixl_mem_t mem_type, int dev_id, size_t len, void*& addr)
{
    switch (mem_type) {
        ...
        case VRAM_SEG: {
            ...
            std::cout << "CUDA addr: " << std::hex << addr << " dev=" << std::dec << dev << " ctx=" << std::hex << ctx
                      << std::dec << std::endl;
            break;
        }
#endif
...
void
allocateWrongGPUTest(nixlBackendEngine* ucx, int dev_id)
{
    nixlBlobDesc desc;
    nixlBackendMD* md;
    void* buf;
...
void
allocateAndRegister(
        nixlBackendEngine* ucx, int dev_id, nixl_mem_t mem_type, void*& addr, size_t len, nixlBackendMD*& md)
{
    nixlBlobDesc desc;
...
void
deallocateAndDeregister(nixlBackendEngine* ucx, int dev_id, nixl_mem_t mem_type, void*& addr, nixlBackendMD*& md)
{
...
void
loadRemote(
        nixlBackendEngine* ucx, int dev_id, std::string agent, nixl_mem_t mem_type, void* addr, size_t len,
...
void
populateDescs(nixl_meta_dlist_t& descs, int dev_id, void* addr, int desc_cnt, size_t desc_size, nixlBackendMD*& md)
{
...
void
performTransfer(
        nixlBackendEngine* ucx1, nixlBackendEngine* ucx2, nixl_meta_dlist_t& req_src_descs,
        nixl_meta_dlist_t& req_dst_descs, void* addr1, void* addr2, size_t len, nixl_xfer_op_t op, bool progress,
        bool use_notif)
{
    int ret2;
    nixl_status_t ret3;
...
void
test_intra_agent_transfer(bool p_thread, nixlBackendEngine* ucx, nixl_mem_t mem_type)
{
    ...
    for (size_t i = 0; i < sizeof(ops) / sizeof(ops[i]); i++) {
        for (bool use_notif : use_notifs) {
            ...
            for (int k = 0; k < iter; k++) {
                ...
                /* Test */
                performTransfer(ucx, ucx, req_src_descs, req_dst_descs, addr1, addr2, len, ops[i], p_thread, use_notif);
            }
        }
    }
...
test_inter_agent_transfer(
        bool p_thread, nixlBackendEngine* ucx1, nixl_mem_t src_mem_type, int src_dev_id, nixlBackendEngine* ucx2,
        nixl_mem_t dst_mem_type, int dst_dev_id)
{
    int ret;
    int iter = 10;
    ...
    std::cout << std::endl << std::endl;
    std::cout << "****************************************************" << std::endl;
    std::cout << "    Inter-agent memory transfer test P-Thr=" << (p_thread ? "ON" : "OFF") << std::endl;
    std::cout << "         (" << memType2Str(src_mem_type) << " -> " << memType2Str(dst_mem_type) << ")" << std::endl;
    std::cout << "****************************************************" << std::endl;
    std::cout << std::endl << std::endl;
    ...
    for (size_t i = 0; i < sizeof(ops) / sizeof(ops[i]); i++) {
        for (bool use_notif : use_notifs) {
            ...
            for (int k = 0; k < iter; k++) {
                ...
                /* Test */
                performTransfer(
                        ucx1, ucx2, req_src_descs, req_dst_descs, addr1, addr2, len, ops[i], !p_thread, use_notif);
            }
        }
    }
...
#ifdef HAVE_CUDA
        if (n_vram_dev > 1) {
            test_inter_agent_transfer(thread_on[i], ucx[i][0], VRAM_SEG, dev_ids[0], ucx[i][1], VRAM_SEG, dev_ids[1]);
            test_inter_agent_transfer(thread_on[i], ucx[i][0], DRAM_SEG, dev_ids[0], ucx[i][1], VRAM_SEG, dev_ids[1]);
            test_inter_agent_transfer(thread_on[i], ucx[i][0], VRAM_SEG, dev_ids[0], ucx[i][1], DRAM_SEG, dev_ids[1]);
        }
#endif
    }
