...
nixlBackendEngine*
createEngine(std::string name, uint32_t ndev, bool p_thread)
{
...
int
dev_distr_rr(int idx, int max_idx, int cnt)
{
    return idx % cnt;
}

int
dev_distr_blk(int idx, int max_idx, int cnt)
{
...
void
createLocalDescs(
        nixlBackendEngine* ucx, nixl_meta_dlist_t& descs, int dev_cnt, dev_distr_t dist_f, int desc_cnt,
        size_t desc_size)
{
...
void
destroyLocalDescs(nixlBackendEngine* ucx, nixl_meta_dlist_t& descs)
{
...
void
createRemoteDescs(
        nixlBackendEngine* src_ucx, std::string agent, nixl_meta_dlist_t& src_descs, nixlBackendEngine* dst_ucx,
...
void
destroyRemoteDescs(nixlBackendEngine* dst_ucx, nixl_meta_dlist_t& dst_descs)
{
    nixl_status_t status;
...
performTransfer(
        ...
        nixl_meta_dlist_t& req_dst_descs, nixl_xfer_op_t op, bool progress, bool use_notif)
{
    nixl_status_t status;
...
void
test_agent_transfer(
        bool p_thread, nixlBackendEngine* ucx1, nixl_mem_t src_mem_type, int src_dev_cnt, dev_distr_t src_dist_f,
    ...
    for (size_t i = 0; i < sizeof(ops) / sizeof(ops[i]); i++) {
        for (bool use_notif : use_notifs) {
            ...
            for (int k = 0; k < iter; k++) {
                ...
                /* Test */
                performTransfer(ucx1, ucx2, ucx1_src_descs, ucx1_dst_descs, ops[i], !p_thread, use_notif);
            }
        }
    }
...
#define NUM_WORKERS 8
    ...
    for (size_t i = 0; i < THREAD_ON_SIZE; i++) {
        ...
        test_agent_transfer(
                thread_on[i], ucx[i][0], DRAM_SEG, ndevices, dev_distr_rr, ucx[i][0], DRAM_SEG, ndevices,
                dev_distr_blk);
#ifdef HAVE_CUDA
        if (n_vram_dev) {
            test_agent_transfer(
                    thread_on[i], ucx[i][0], VRAM_SEG, ndevices, dev_distr_rr, ucx[i][0], VRAM_SEG, ndevices,
                    dev_distr_blk);
        }
#endif
    }
    ...
    for (size_t i = 0; i < THREAD_ON_SIZE; i++) {
        test_agent_transfer(
                thread_on[i], ucx[i][0], DRAM_SEG, ndevices, dev_distr_rr, ucx[i][1], DRAM_SEG, ndevices,
                dev_distr_blk);

#ifdef HAVE_CUDA
        if (n_vram_dev) {
            test_agent_transfer(
                    thread_on[i], ucx[i][0], VRAM_SEG, n_vram_dev, dev_distr_rr, ucx[i][1], VRAM_SEG, n_vram_dev,
                    dev_distr_blk);
            test_agent_transfer(
                    thread_on[i], ucx[i][0], VRAM_SEG, n_vram_dev, dev_distr_rr, ucx[i][1], VRAM_SEG, n_vram_dev,
                    dev_distr_blk);
            test_agent_transfer(
                    thread_on[i], ucx[i][0], VRAM_SEG, n_vram_dev, dev_distr_rr, ucx[i][1], DRAM_SEG, n_vram_dev,
                    dev_distr_blk);
            test_agent_transfer(
                    thread_on[i], ucx[i][0], DRAM_SEG, n_vram_dev, dev_distr_rr, ucx[i][1], VRAM_SEG, n_vram_dev,
                    dev_distr_blk);
        }
#endif
    }
