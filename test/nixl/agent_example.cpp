...
test_side_perf(nixlAgent* A1, nixlAgent* A2, nixlBackendH* backend, nixlBackendH* backend2)
{
    ...
    std::cout << "prepXferDlist, total time for " << n_iters << " iters: " << diff_time.tv_sec << "s "
...
sideXferTest(nixlAgent* A1, nixlAgent* A2, nixlXferReqH* src_handle, nixlBackendH* dst_backend)
{
    ...
    while (xfer_status != NIXL_SUCCESS) {
        if (xfer_status != NIXL_SUCCESS)
            xfer_status = A1->getXferStatus(req1);
    ...
    while (xfer_status != NIXL_SUCCESS) {
        if (xfer_status != NIXL_SUCCESS)
            xfer_status = A1->getXferStatus(req2);
        assert(xfer_status >= 0);
    }
    ...
    while (xfer_status != NIXL_SUCCESS) {
        if (xfer_status != NIXL_SUCCESS)
            xfer_status = A1->getXferStatus(req3);
        assert(xfer_status >= 0);
    }
...
agent_example()
{
    ...
    while (status != NIXL_SUCCESS || n_notifs == 0) {
        if (status != NIXL_SUCCESS)
            status = A1.getXferStatus(req_handle2);
        if (n_notifs == 0)
            ret2 = A1.getNotifs(notif_map);
        assert(status >= 0);
        assert(ret2 == NIXL_SUCCESS);
        n_notifs = notif_map.size();
    }
